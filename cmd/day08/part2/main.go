...
func junctionBoxes(filePath string) float64 {
	boxes := Boxes{}
	pairs := Pairs{}
	...
	utils.Debug("lastPair", lastPair)
	utils.Debug("a", boxes[lastPair.a])
	utils.Debug("b", boxes[lastPair.b])

	return boxes[lastPair.a].x * boxes[lastPair.b].x
}

func main() {
	result := junctionBoxes("../input.txt")
	fmt.Print("RESULT ---\n")
	fmt.Printf("RESULT: %f\n", result)
	fmt.Print("RESULT ---\n")
}
